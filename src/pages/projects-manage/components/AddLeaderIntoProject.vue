  ...
  <div class="modal-overlay" @click.self="close">
    <div class="modal-container">
      <div class="modal-header d-flex justify-content-between">
        <h2 class="modal-title">Thêm Leader vào dự án {{ project.name }}</h2>
        <button class="btn-close text-end" @click="close"></button>
      </div>
      <div class="search-container">
        <input type="text" v-model="tableSearchQuery" placeholder="Tìm kiếm tên nhân viên hoặc bộ phận hoặc chức vụ"
          class="search-bar" />
      </div>
          ...
          <tbody>
            <tr v-for="(item, index) in paginatedEmployees" :key="index">
              <td class="text-center">
                <input class="form-check-input" type="checkbox" :checked="selectedEmployee === index"
                  @click="toggleEmployee(index)" />
              </td>

              <td>
                <img :src="item.fileInfo ? item.fileInfo.fileUrl : defaultImg" class="employee-img" />
              </td>
              <td>{{ item.name }}</td>
              <td>
                <!-- {{
      ...
      <div class="buttons">
        <button class="btn btn-success" @click="updateProject">Xác nhận</button>
      </div>
      <div class="pagination">
        ...
        <button @click="nextPage" :disabled="currentPage === totalEmployeePages" class="pagination-btn">
...
export default {
  ...
  data() {
    return {
      ...
      tableSearchQuery: "",
      selectedEmployee: null, // Store selected employee indices
      currentPage: 1,
      pageSize1: 10,
      employees: [],
  ...
  methods: {
    ...
    async updateProject() {
      ...
      const updatedProject = { leaderId: employeeId };
    ...
    toggleEmployee(index) {
      this.selectedEmployee = this.selectedEmployee === index ? null : index;
    },
    prevPage() {
